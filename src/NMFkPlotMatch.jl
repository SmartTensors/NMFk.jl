...
function plotmatches(X::AbstractArray, Xest::AbstractArray, timebins::AbstractVector, attributes::AbstractVector, locations::AbstractVector; figuredir::AbstractString=".", filename::AbstractString="", combine::Bool=false, dpi::Int64=300, background_color=nothing, hsize::Measures.AbsoluteLength=8Compose.inch, vsize::Measures.AbsoluteLength=4Compose.inch, quiet::Bool=false)
	...
	for i in axes(Xv1, 1)
		...
		p1 = Mads.plotseries(Xv1[i, :, :]; title=locations[i], xaxis=timebins, name = "", names=combine ? fill("", ns) : attributes, key_position=combine ? :none : :right, ymin=0, ymax=m, hsize=hsize, vsize=vsize, plotline=!combine, quiet=true, code=combine, returnplot=!combine)
		p2 = Mads.plotseries(Xv2[i, :, :]; title="", xaxis=timebins, names=attributes, ymin=0, ymax=m, hsize=hsize, vsize=vsize, quiet=true, code=combine, returnplot=!combine)
		if combine
			p = Gadfly.plot(p1..., p2...)
			vsize_new = vsize
		else
